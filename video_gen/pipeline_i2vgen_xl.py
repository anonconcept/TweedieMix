...
from video_gen.utils_attn import *
import numpy as np
import PIL
import torch
...
class I2VGenXLPipeline(
    ...
    def __call__(
        ...
        injection_timestep = int(num_inference_steps*injection_timestep)
        injection_timestep = self.scheduler.timesteps[:injection_timestep] if injection_timestep >= 0 else []
        # register_attention_control_efficient(self, injection_timestep)
        register_conv_control_efficient(self, injection_timestep,interp_ratio)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                eps = at.sqrt()*noise_pred+ (1-at).sqrt()*latents
                
                denoised_tweedie= self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs).pred_original_sample

                denoised_tweedie= at.sqrt()*latents-(1-at).sqrt()*noise_pred
                
                latents = at_next.sqrt() * denoised_tweedie + (1-at_next).sqrt() * eps
